...
<form id="ContactForm" action="personAddr/contactForm.json" method="post">
	<div id="ContactBox" class="gWel-tab">
		...
		<div class="pans">
				...
				<table border="0" cellpadding="0" cellspacing="0"  title="个人具体信息">
					...
					<tr>
						...
						<td class="tContentcls">
							<select <c:if test="${isDetail }">readonly class="readonly"</c:if> name="sex" id="sex">
			...
			<div id="companyTIDc" class="panel">
				<table border="0" cellpadding="0" cellspacing="0"  title="单位具体信息">
					...
					<tr>
					  ...
					  <td class="tContentcls"><input <c:if test="${isDetail }">readonly class="readonly"</c:if> name="facsimiletelephonenumber" id="facsimiletelephonenumber" type="text" value="<c:out value='${psn.facsimiletelephonenumber}'/>"></td>
					</tr>
					<tr>
						<td class="tTitlecls">商务电话1：</td>
					  	<td class="tContentcls"><input <c:if test="${isDetail }">readonly class="readonly"</c:if> name="phone" id="phone" type="text" value="<c:out value='${psn.phone}'/>"></td>
						<td class="tTitlecls">商务电话2：</td>
					  	<td class="tContentcls"><input <c:if test="${isDetail }">readonly class="readonly"</c:if> name="telephonenumber" id="telephonenumber" type="text" value="<c:out value='${psn.telephonenumber}'/>"></td>
				  	</tr>
				</table>
			</div>
			...
			<div id="remarkTIDc" class="panel">
				<table border="0" cellpadding="0" cellspacing="0" title="备注信息" >
					<tr>
						<td><textarea <c:if test="${isDetail }">readonly class="readonly"</c:if> name="remark" id="remark" cols="80" rows="8"><c:out value='${psn.remark}'/></textarea></td>
					</tr>
				</table>
			</div>
		</div>
		...
		<div class="iTab_btn theme-bg-col">
	  		<input type="hidden" name="uuid" value="<c:out value='${psn.uuid}'/>">
				<input type="hidden" name="type" value="<c:out value='${type}'/>">
				<c:if test="${groupid eq 'nogroup'}">
					<c:set var="groupid_" value=""/>
				</c:if>
				<c:if test="${groupid ne 'nogroup' && not empty groupid}">
					<c:set var="groupid_" value="${groupid}"></c:set>
				</c:if>
	  		<input type="hidden" name="groupid" value="<c:out value='${groupid_}'/>">
			<c:if test="${type eq 'add' || type eq 'mod'}">
				<input id="ContactForm-submit" type="submit" value="提交">
			</c:if>
			<c:if test="${type eq 'view'}">
				<input type="button" value="修改" onclick="javascript:PersonAddr.toMod('<c:out value='${groupid}'/>', '<c:out value='${psn.uuid}'/>');">
			</c:if>
			<input type="button" value="返回" onclick="javascript:PersonAddr.open('<c:out value='${groupid}'/>');">
  		</div>
	</div>
</form>
